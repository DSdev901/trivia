...
function ensureNavLayer(host) {
  let nav = host.querySelector(".geo-map-nav");
  if (nav) return nav;
  nav = document.createElement("div");
  nav.className = "geo-map-nav";
...
function bindMapControls(host) {
  ...
  const unbindWin = () => {
    ...
    window.removeEventListener("lostpointercapture", onPtrEnd);
  ...
  const dropExtraTouches = (touchList) => {
    if (!contacts.size) return;
    ...
    if (contacts.size <= touches.length) return;
    const used = new Set();
    const keep = new Map();
    for (const t of touches) {
      ...
      used.add(bestId);
      keep.set(bestId, { x: t.clientX, y: t.clientY });
    }
    contacts.clear();
    for (const [id, p] of keep) contacts.set(id, p);
  };
  const scheduleNav = () => {
    if (navRaf) return;
  ...
  const onWinTouchMove = (e) => {
    ...
    for (const t of e.changedTouches) {
      ...
      if (bestId == null) continue;
      used.add(bestId);
      contacts.set(bestId, { x: t.clientX, y: t.clientY });
    }
    scheduleNav();
  };
  const onWinTouchEnd = (e) => {
    if (!contacts.size) return;
    dropExtraTouches(e.touches);
    flushNav();
    settleFingers();
  ...
  const bindWin = () => {
    ...
    window.addEventListener("lostpointercapture", onPtrEnd);
  ...
  /* Overlay is the touch target so island/hit-pad rebuilds cannot cancel the
     gesture. Window listeners keep move/end if Safari retargets. */
  nav.addEventListener("pointerdown", (e) => {
    if (e.button !== 0) return;
    if (e.pointerType === "touch") lastTouchAt = performance.now();
    if (e.pointerType === "mouse" && performance.now() - lastTouchAt < 800) return;
    clearTimeout(finishTimer);
    finishTimer = 0;
    stopFocusZoom();
    if (e.isPrimary) {
      for (const id of [...contacts.keys()]) {
        ...
        let live = false;
        try {
          live = nav.hasPointerCapture(id);
        } catch {
          live = false;
        }
        if (!live) contacts.delete(id);
      }
    }
    contacts.set(e.pointerId, { x: e.clientX, y: e.clientY });
    capturePtr(nav, e);
  ...
  nav.addEventListener("pointermove", onPtrMove);
  nav.addEventListener("pointerup", onPtrEnd);
  nav.addEventListener("pointercancel", onPtrEnd);
  nav.addEventListener("lostpointercapture", onPtrEnd);
  const vis = () => {
    if (!host.isConnected) {
...
function packTitlePrefixes(sectionName) {
  ...
  const prefixes = [`${name}:`];
  if (name === "United States") prefixes.push("The U.S.:", "U.S.:");
  if (name === "Caribbean") prefixes.push("The Caribbean:");
  if (name === "Australia & New Zealand") {
    prefixes.push("Australia:", "Australia and New Zealand:");
  }
  return prefixes;
}

function packTitleInSection(pack, sectionName) {
  const name = String(pack?.name || "");
  for (const prefix of packTitlePrefixes(sectionName)) {
    if (name.startsWith(prefix)) return name.slice(prefix.length).trim() || name;
  }
  if (sectionName && name === sectionName) {
    if (pack.quiz === "countries") return "Countries";
    if (pack.quiz === "capitals") return "Capitals";
    if (pack.quiz === "flags") return "Flags";
    if (pack.quiz === "outlines") return "Outlines";
  }
  return name;
}
...
const COMPACT_PLAY_MQ =
  "(max-width: 860px), (hover: none) and (max-height: 640px)";
...
function shieldMapFromFocusClick(mapWrap) {
  ...
  window.setTimeout(() => {
    if (mapWrap.isConnected) mapWrap.style.pointerEvents = "";
  }, 700);
}
