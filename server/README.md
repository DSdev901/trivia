...
3. Set `DATABASE_URL`, `UPLOAD_PIN`, `ALLOWED_ORIGINS` (`https://dsdev901.github.io`), and optionally `ANTHROPIC_API_KEY` so photo parsing uses a vision model instead of OCR. Set `HIT_SKIP_IPS` to your public IP (comma-separated) so your own visits do not increment the home counter.
...
`GET`/`POST /api/hits` is the shared home-page visitor tally. No PIN. The same IP is counted at most once every 30 minutes. IPs in `HIT_SKIP_IPS` are shown the current count but never added.
