...
async function D(e) {
  if (e.match("https://huggingface.co"))
    try {
      ...
      const file = await (await n.getFileHandle(o)).getFile();
      if (!file || file.size < 32) return;
      return file;
    } catch {
      return;
    }
...
async function S(e, m) {
  ...
  if (!n.ok) throw new Error("Could not download voice model (" + n.status + ")");
...
function bindOrt(mod) {
  if (mod && mod.InferenceSession && mod.Tensor) return mod;
  if (mod && mod.default && mod.default.InferenceSession) return mod.default;
  return mod;
}

async function ensureOrt() {
  h = h ?? await import("./piper-DeOu3H9E.js");
  if (!_) {
    const mod = await import("https://cdn.jsdelivr.net/npm/onnxruntime-web@1.18.0/dist/esm/ort.wasm.min.js");
    _ = bindOrt(mod);
  }
  _.env.allowLocalModels = false;
  // GitHub Pages is not cross-origin isolated, so threaded WASM cannot use SharedArrayBuffer.
  _.env.wasm.numThreads = 1;
  _.env.wasm.proxy = false;
...
async function N(e, m) {
  await ensureOrt();
  const n = c[e.voiceId];
  if (!n) throw new Error("Unknown Piper voice");
  const o = JSON.stringify([{ text: String(e.text || "").trim() }]);
  if (!cachedConfig || cachedVoiceId !== e.voiceId) {
    const a = await f(`${u}/${n}.json`);
    cachedConfig = JSON.parse(await a.text());
  }
  const i = cachedConfig;
  const t = await new Promise(async (v) => {
  ...
  const r = 0, s = i.audio.sample_rate, d = i.inference.noise_scale, g = i.inference.length_scale, U = i.inference.noise_w;
  if (!cachedSession || cachedVoiceId !== e.voiceId) {
    if (cachedSession) {
      try { await cachedSession.release(); } catch { /* ignore */ }
      cachedSession = null;
    }
    const k = await f(`${u}/${n}`, m);
    cachedSession = await _.InferenceSession.create(await k.arrayBuffer());
    cachedVoiceId = e.voiceId;
  }
  const y = cachedSession;
  const w = {
...
async function I(e, m) {
  ...
  await Promise.all(
    o.map((a) => f(a, a.endsWith(".onnx") ? m : void 0))
  );
}
async function F(e) {
